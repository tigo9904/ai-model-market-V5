...
export async function uploadProductImagesAction(base64Images: string[]): Promise<UploadResult> {
  console.log("[Server Action] uploadProductImagesAction: Received", base64Images.length, "images for upload.")
  ...
  try {
    for (let i = 0; i < base64Images.length; i++) {
      ...
      try {
        ...
        // Potentially skip this image or add to a list of failed images
        continue
      }
      ...
      const blobResult = await put(uniqueFileName, buffer, {
        ...
        // token: process.env.BLOB_READ_WRITE_TOKEN, // `put` should pick this up automatically from env on server
      })
    ...
    // Check for specific Vercel Blob errors if possible (error structure might vary)
